...
class GaussianModel:
    ...
    def setup_functions(self):
        def build_covariance_from_scaling_rotation(center, scaling, scaling_modifier, rotation):
            RS = build_scaling_rotation(torch.cat(
                [scaling * scaling_modifier, torch.ones_like(scaling)], dim=-1), rotation).permute(0, 2, 1)
    ...
    def set_instance_feature(self, instacne_features):
        assert instacne_features.shape[0] == self._opacity.shape[0]
        self._instance_feature = instacne_features
    ...
    def training_setup(self, training_args):
        ...
        if training_args.include_feature:
            if self._instance_feature is None or self._instance_feature.shape[0] != self._xyz.shape[0]:
                ...
                instance_feature = torch.randn((self._xyz.shape[0], 16), device="cuda")
    ...
    def load_ply(self, path):
        ...
        self._xyz = nn.Parameter(torch.tensor(xyz, dtype=torch.float,
    ...
    def prune_points(self, mask):  # mark
        ...
        try:
            ...
            print("Exception in max_radii2D pruning: ", e)
            self.max_radii2D = torch.zeros((self.get_xyz.shape[0]), device="cuda")
    ...
    def weights_split(self, split_mask, scene_extent, scale=0.8, N=2):
        # split_mask=torch.logical_and(split_mask,torch.max(self.get_scaling, dim=1).values > self.percent_dense*scene_extent)
        ...
        new_scaling = self.scaling_inverse_activation(
            self.get_scaling[selected_pts_mask].repeat(N, 1) / (scale*N))
    ...
    def weights_prune(self, sp_mask, pre_sp_num, strategy=1):
        p_mask = torch.zeros_like(sp_mask)
        if strategy == 1:  
            pa = sp_mask[-pre_sp_num:]
            pb = sp_mask[-2*pre_sp_num:-pre_sp_num]
            p_mask[-pre_sp_num:] = pa & pb
        elif strategy == 2: 
            p_mask[-2*pre_sp_num:] = sp_mask[-2*pre_sp_num:]
            pa = sp_mask[-pre_sp_num:]
            pb = sp_mask[-2*pre_sp_num:-pre_sp_num]
            p_mask[-pre_sp_num:] = p_mask[-pre_sp_num:] & ~(pa & pb)
        elif strategy == 3: 
            p_mask[-2*pre_sp_num:] = sp_mask[-2*pre_sp_num:]
        else:  
            p_mask = sp_mask
        self.prune_points(p_mask)
    ...
    def add_densification_stats(self, viewspace_point_tensor, update_filter):
        self.xyz_gradient_accum[update_filter] += torch.norm(
            viewspace_point_tensor.grad[update_filter], dim=-1, keepdim=True)  # grad
        self.denom[update_filter] += 1  # nums
