...
def render(viewpoint_camera, pc: GaussianModel, pipe, bg_color: torch.Tensor, include_feature=False, scaling_modifier=1.0, override_color=None, renderid=False):
    """
    Render the scene. 
    ...
    if pipe.compute_cov3D_python:
        ...
        cov3D_precomp = (splat2world[:, [0, 1, 3]] @ world2pix[:, [0, 1, 3]]
                         ).permute(0, 2, 1).reshape(-1, 9)  # column major
    else:
    ...
    if include_feature:
        if not renderid:
            ...
            instance_feature_precomp = instance_feature_precomp / \
                (instance_feature_precomp.norm(dim=-1, keepdim=True) + 1e-9)
            # instance_feature_precomp = torch.sigmoid(instance_feature_precomp)
        else:
            ids = torch.from_numpy(pc.get_ids)
            if ids.max() > 15:
                raise ()
            instance_feature_precomp = torch.nn.functional.one_hot(
                ids, num_classes=16).to(torch.float).cuda()
    else:
        instance_feature_precomp = torch.zeros(
            (1,), dtype=opacity.dtype, device=opacity.device)  # lang
