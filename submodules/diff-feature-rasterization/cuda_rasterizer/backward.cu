...
renderCUDA(
	...
	const float* __restrict__ instance_feature,
	const float* __restrict__ depths,
	const float* __restrict__ final_Ts,
	const uint32_t* __restrict__ n_contrib,
	const float* __restrict__ dL_dpixels,
	const float* __restrict__ dL_dpixels_F,//add
	...
	bool include_feature)
{
	...
	float last_instance_feature[F] = {0};
	if (include_feature) {
		if (inside)
			for (int i = 0; i < F; i++)
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		if (range.x + progress < range.y)
		{
			...
			for (int i = 0; i < F; i++)
				collected_feature[i * BLOCK_SIZE + block.thread_rank()] = instance_feature[coll_id * F + i];
		}
		block.sync();
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			if (include_feature) {
				for (int ch = 0; ch < F; ch++)
				{
					...
					accum_rec_F[ch] = last_alpha * last_instance_feature[ch] + (1.f - last_alpha) * accum_rec_F[ch];
					last_instance_feature[ch] = f;
					...
					// Atomic, since this pixel is just one of potentially
					// many that were affected by this Gaussian.
					atomicAdd(&(dL_dinstance_feature[global_id * F + ch]), dchannel_dcolor * dL_dchannel_F);
				}
			}//add
...
void BACKWARD::render(
	...
	bool include_feature)
{
	renderCUDA<NUM_CHANNELS, NUM_CHANNELS_instance_feature> << <grid, block >> >(
		ranges,
		point_list,
		W, H,
		focal_x, focal_y,
