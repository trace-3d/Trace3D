...
template <uint32_t CHANNELS,uint32_t CHANNELS_instance_feature>
...
renderCUDA(
	...
	float* __restrict__ out_instance_feature,
	bool include_feature)
{
	...
	float C[CHANNELS] = { 0 };
	float F[CHANNELS_instance_feature] = { 0 };
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			if (include_feature){
				for (int ch = 0; ch < CHANNELS_instance_feature; ch++)
					F[ch] += instance_feature[collected_id[j] * CHANNELS_instance_feature + ch] * w;
			}
			T = test_T;
	...
	if (inside)
	{
		...
		if (include_feature) {
			for (int ch = 0; ch < CHANNELS_instance_feature; ch++)
				out_instance_feature[ch * H * W + pix_id] = F[ch]; 
		}
